...
def string_pallyndrome_checker(string_custom):
    ...
    if d == e:
        ...
        return "the string given by the user is not a pallyndrome"
...
def number_pallyndrome_checker(custom_num):
    ...
    f = list(reversed(e))
    if e == f:
        return "the number given is a pallyndrome"
    else:
        return "the number given is not a pallyndrome"
...
def main():
    ...
    if operator.lower() == "string_pal":
        ...
        print(string_pallyndrome_checker(string_custom))
    elif operator.lower() == "num_pal":
        custom_num = int(input("Enter your desired number over here"))
        print(number_pallyndrome_checker(custom_num))
    else:
...
# main iterative body
for i in range(5):
    choice = input("Wish to continue ? ----> yes/no").strip()
    if choice.lower() == "yes":
        main()
